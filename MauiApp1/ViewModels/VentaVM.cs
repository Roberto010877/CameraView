    ...
    public partial class VentaVM : ObservableObject
    {
        ...
        private async Task FinalizarVenta()
        {
            ...
            try
            {
                ...
                {
                    var producto = await _context.Productos.FirstOrDefaultAsync(p => p.IdProducto == item.Producto.IdProducto);
                    if (producto != null)
                    {
                        if (producto.Cantidad < item.Cantidad)
                        {
                            await Shell.Current.DisplayAlert("Error!", $"No hay suficiente stock para el producto '{producto.Nombre}'. Stock disponible: {producto.Cantidad}", "Aceptar");
                            return;
                        }
